    ...
    public partial class Secp256k1Benchmarks
    {
        ...
        private void ValidateEcdsaSignatures()
        {
            ...
            foreach (var (name, compactSig) in signatures)
            {
                ...
                if (!secp256k1.EcdsaSignatureParseCompact(parsedSig, compactSig))
                    throw new Exception($"EcdsaSign validation failed: {name} signature could not be parsed");
        ...
        private void ValidateSchnorrSignatures()
        {
            ...
            {
                if (!secp256k1.SchnorrsigVerify(sig, inputs.Msg.MsgHash, xOnlyPubKey))
                {
