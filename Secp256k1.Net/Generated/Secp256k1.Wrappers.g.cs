...
namespace Secp256k1Net
{
    ...
    /// <summary>Flags for public key serialization format.</summary>
    public enum Secp256k1EcFlags : uint
    {
        /// <summary>Compressed format (33 bytes).</summary>
        Compressed = 258,
        /// <summary>Uncompressed format (65 bytes).</summary>
        Uncompressed = 2,
    }

    /// <summary>Flags for secp256k1 context creation.</summary>
    public enum Secp256k1ContextFlags : uint
    {
        /// <summary>Creates a context sufficient for all functionality.</summary>
        None = 1,
    }

    /// <summary>A pointer to a function to deterministically generate a nonce.</summary>
    /// <param name="nonce32">pointer to a 32-byte array to be filled by the function.</param>
    /// <param name="msg32">the 32-byte message hash being verified (will not be NULL)</param>
    /// <param name="key32">pointer to a 32-byte secret key (will not be NULL)</param>
    /// <param name="algo16">pointer to a 16-byte array describing the signature algorithm (will be NULL for ECDSA for compatibility).</param>
    /// <param name="data">Arbitrary data pointer that is passed through.</param>
    /// <param name="attempt">how many iterations we have tried to find a nonce. This will almost always be 0, but different attempt values are required to result in a different nonce.<para>Except for test cases, this function should compute some cryptographic hash of the message, the algorithm, the key and the attempt.</para></param>
    ...
    /// <param name="x32">pointer to a 32-byte x coordinate</param>
    ...
    /// <param name="nonce32">pointer to a 32-byte array to be filled by the function</param>
    /// <param name="msg">the message being verified. Is NULL if and only if msglen is 0.</param>
    /// <param name="msglen">the length of the message</param>
    /// <param name="key32">pointer to a 32-byte secret key (will not be NULL)</param>
    /// <param name="xonly_pk32">the 32-byte serialized xonly pubkey corresponding to key32 (will not be NULL)</param>
    /// <param name="algo">pointer to an array describing the signature algorithm (will not be NULL)</param>
    /// <param name="algolen">the length of the algo array</param>
    /// <param name="data">arbitrary data pointer that is passed through<para>Except for test cases, this function should compute some cryptographic hash of the message, the key, the pubkey, the algorithm description, and data.</para></param>
    /// <returns>1 on success, 0 on failure.</returns>
    ...
    /// <param name="output">pointer to an array to be filled by the function</param>
    /// <param name="x32">pointer to the 32-byte serialized X coordinate of the resulting shared point (will not be NULL)</param>
